...
<div class="wrap km-recipes-wrap">
    ...
    <div class="km-header-bar">
        ...
        <p style="margin: 4px 0 0 0; color: #646970;">Lance insumos na medida exata de preparo (g, kg, ml, L ou embalagem inteira) com conversão automática.</p>
    </div>
        ...
        <div class="km-card km-form-col">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="km-recipe-form">
                ...
                <div class="km-field">
                    ...
                    <input type="text" id="km-r-name" name="name" value="<?php echo esc_attr($editName); ?>" placeholder="Ex: Brigadeiro Tradicional, Massa de Pão de Ló..." required class="km-input-text">
                </div>

                <div class="km-field">
                    <label for="km-r-yield">Rendimento da Receita (unidades/porções):</label>
                    <input type="number" id="km-r-yield" name="yield" value="<?php echo esc_attr($editYield); ?>" step="0.1" min="0.1" required class="km-input-text">
                                ...
                                <div class="km-tags-flex">
                                    <?php if (!empty($rItems) && is_iterable($rItems)): ?>
                                        <?php foreach ($rItems as $it): 
                                            ...
                                            $itCost = (float) km_get_prop($it, ['cost', 'total_cost', 'totalCost'], 0.0);
                                            $itMeasure = km_get_prop($it, ['measure_type', 'measureType', 'unit_type'], 'unit');
                                            $itUnit = km_get_prop($it, ['unit', 'usage_unit', 'usageUnit'], 'g');
                                            $itPkgType = km_get_prop($it, ['package_type', 'pkg_type'], 'Embalagem');
                                            $itId   = (int) km_get_prop($it, ['id'], 0);
                                            
                                            $labelDisplay = "{$itQty} {$itUnit}";
                                            if ($itMeasure === 'pkg') {
                                                $labelDisplay = "{$itQty}x {$itPkgType}";
                                            } elseif ($itMeasure === 'g_from_kg') {
                                                $labelDisplay = "{$itQty} g";
                                            } elseif ($itMeasure === 'ml_from_l') {
            ...
            div.innerHTML = `
                ...
                <div class="km-item-row-flex">
                    <div class="km-qty-box">
                        <label style="font-size: 11px; text-transform: uppercase; color: #646970; font-weight: 700;">Qtd e Medida:</label>
                        <div class="km-inline-inputs">
                            <input type="number" name="items[${idx}][quantity]" value="1" step="0.0001" min="0.0001" class="km-input-qty" required>
